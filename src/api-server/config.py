...
class Settings:
    ...
    def __init__(self) -> None:
        ...
        self.header_key = "XteNATqxnbBkPa6TCHcK0NTxOM1JVkQl"  # Fixed secret key expected from the frontend
        self.algorithm = "HS256"  # Algorithm used for encoding JWT
        self.is_https = os.getenv("USE_HTTPS", "false").strip().lower() == "true"

        # Token settings
        token_expiry_hours = self._get_env_int("TOKEN_EXPIRY_IN_HOURS", 24)
        self.access_token_expire_minutes = token_expiry_hours * 60  # Convert hours to minutes
